...
let tipoEnvio = parseInt(prompt("🛒 El total de tu compra es $"+totalProducto+" (sin IVA) \n¿Deseas incluir el despacho? También puedes retirar en nuestra tienda ubicada en la calle MarkBarker 182, Nepal.\n1- Incluir despacho normal \n2-Despacho Express\n3-Retiro en Tienda"));

if(tipoEnvio == 1){
    alert("💰 Total a pagar $"+valorFinal(IVA,envioNormal));
}else if(tipoEnvio == 2){
    alert("💰 Total a pagar $"+valorFinal(IVA,envioExpress));
}else if(tipoEnvio == 3){
    alert("💰 Total a pagar $"+valorFinal(IVA,0));
}
